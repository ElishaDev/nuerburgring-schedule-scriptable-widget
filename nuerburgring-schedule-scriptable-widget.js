...
async function fetchOpenHours() {
  ...
  const scheduleMatch = html.match(/data-location-id="12"[^>]*data-schedule="([^"]+)"/);
  if (!scheduleMatch) {
    throw new Error("Nordschleife schedule data not found");
  }

  const scheduleJson = JSON.parse(decodeHtmlEntities(scheduleMatch[1]));
...
function formatStatus(entry) {
  ...
  return {
    text: message ? `${times} (${message})` : `Open: ${times}`,
    color: Color.green(),
  };
}
...
async function main() {
  try {
    ...
    const title = widget.addText("Nordschleife");
    title.font = Font.boldSystemFont(12);
    title.textColor = Color.white();
